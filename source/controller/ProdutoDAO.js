...
class ProdutoDAO {
  ...
  async gravar(obj) {
    try {
      Banco.init();
      await Banco.conexao.query('INSERT INTO produto(descricao,preco,qtde) VALUES($1,$2,$3) RETURNING *', [obj.descricao, obj.preco,
        obj.qtde])
        .then(resposta => {
  ...
  async alterar(obj) {
    try {
      Banco.init();
      await Banco.conexao.query('Update produto set descricao=$1,preco=$2, qtde=$3 where codigo=$4', [obj.descricao, 
        obj.preco, obj.qtde, obj.codigo])
        .then(_resposta => {
